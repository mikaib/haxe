...
let dump_types com pretty =
	...
	let f mt =
		...
		| Type.TEnumDecl e ->
			...
			print "}"
		| Type.TTypeDecl t ->
			print "%s%stypedef %s%s = %s" (s_metas t.t_meta "") (if t.t_private then "private " else "") (s_type_path path) (params t.t_params) (s_type (Type.follow t.t_type));
		| Type.TAbstractDecl a ->
			...
			(String.concat " " (List.map (fun t -> " from " ^ s_type (Type.follow t)) a.a_from))
			(String.concat " " (List.map (fun t -> " to " ^ s_type (Type.follow t)) a.a_to));
		);
		close()
	in
